# Databricks notebook source exported at Tue, 6 Sep 2016 21:36:45 UTC
# MAGIC %md
...
correlated_data_zero_mean = correlated_data.map(lambda y: y - mean_correlated) 
...
correlated_cov = correlated_data_zero_mean.map(lambda y: np.outer(y, y.T)).reduce(lambda a,b: a + b) / correlated_data_zero_mean.count()
...
def estimate_covariance(data):
    ...
    return data.map(lambda y: y - meanmatrix).map(lambda y: np.outer(y, y.T)).reduce(lambda a,b: a + b) / data.count()
...
inds = np.argsort(eig_vecs)
top_component = np.reshape(eig_vecs * inds, [1,4])
top_component = top_component[np.where( top_component != 0)]
print '\ntop principal indices: {0}'.format(inds)
...
correlated_data_scores = correlated_data.map(lambda y: np.dot(top_component, y))
...
def pca(data, k=2):
    ...
    inds = np.argsort(eig_vecs)
    #top_component = eig_vecs * inds
    #top_component = top_component[np.where( top_component != 0)]
    
    scores = data.map(lambda y: np.dot(eig_vecs, y))#.sortBy(lambda a: a, ascending=False)
    return (np.sort(eig_vecs, axis=0), scores, eig_vals[:k])
