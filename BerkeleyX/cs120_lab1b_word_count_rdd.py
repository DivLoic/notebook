# Databricks notebook source exported at Tue, 23 Aug 2016 22:34:44 UTC
# MAGIC %md
...
# TODO: Replace <FILL IN> with appropriate code
uniqueWords = wordsRDD.map(lambda x: (x,1)).reduceByKey(lambda a,b: a+b).count()
print uniqueWords
...
# TODO: Replace <FILL IN> with appropriate code
top15WordsAndCounts = shakeWordsRDD.map(lambda x: (x,1)).reduceByKey(add).takeOrdered(15, lambda x: -x[1])
