# Databricks notebook source exported at Thu, 8 Sep 2016 12:31:44 UTC
# MAGIC %md
...
powerPlantDF = sqlContext.read.format('com.databricks.spark.csv').options(
  delimiter='\t',
  header='true',
  inferschema='true'
...
altPowerPlantDF = sqlContext.read.format('com.databricks.spark.csv').options(
  delimiter='\t',
  header='true',
  schema = customSchema
).load('/databricks-datasets/power-plant/data')
...
# TEST
from databricks_test_helper import *
expected = set([(s, 'string') for s in ('AP', 'AT', 'PE', 'RH', 'V')])
...
seed = 1800009193L
(split20DF, split80DF) = datasetDF.randomSplit([2.0, 8.0], seed)
