# Databricks notebook source exported at Sun, 14 Aug 2016 01:00:37 UTC
# MAGIC %md
...
not200DF = logs_df.where("status != 200")
not200DF.show(10)
...
days_with_hosts = daily_hosts_df.map(lambda x: x["day"]).collect()
hosts = daily_hosts_df.map(lambda x: x["count"]).collect()
#for <FILL IN>:
#  <FILL IN>
...
total_req_per_day_df = logs_df.groupBy(dayofmonth("time").alias("day")).count().selectExpr("day", "count as total")

avg_daily_req_per_host_df = (
  total_req_per_day_df
  .join(daily_hosts_df, "day")
  .select("day", (col("total") / col("count")).alias("avg_reqs_per_host_per_day"))
)
...
days_with_avg = (avg_daily_req_per_host_df.map(lambda x: x.day).collect())
avgs = (avg_daily_req_per_host_df.map(lambda x: x.avg_reqs_per_host_per_day).collect())
#for <FILL IN>:
#  <FILL IN>
...
not_found_df = logs_df.where("status = 404")
...
days_with_errors_404 = errors_by_date_sorted_df.map(lambda x: x["day"]).collect()
errors_404_by_day = errors_by_date_sorted_df.map(lambda x: x["count"]).collect()
#for <FILL IN>:
#  <FILL IN>
...
hours_with_not_found = hour_records_sorted_df.map(lambda x: x["hour"]).collect()
not_found_counts_per_hour = hour_records_sorted_df.map(lambda x: x["count"]).collect()
