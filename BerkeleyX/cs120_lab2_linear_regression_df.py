# Databricks notebook source exported at Mon, 29 Aug 2016 18:56:25 UTC
# MAGIC %md
...
def lineSplitVector(row):
  figures = row['value'].split(",")
  point = LabeledPoint(figures[0], figures[1:])
  return Row(label=point.label, features=point.features)

# COMMAND ----------

#raw_data_df.printSchema()
#(parsed_points_df.rdd.map(lambda lp: lp.features)).take(5)
...
def parse_points(df):
    ...
    """
    return df.rdd.map(lambda r: lineSplitVector(r)).toDF()
...
# TODO: Replace <FILL IN> with appropriate code
content_stats = (parsed_points_df.rdd
                .map(lambda r: r["label"]).collect())

min_year = min(content_stats)
max_year = max(content_stats)
...
# TODO: Replace <FILL IN> with appropriate code
parsed_data_df = parsed_points_df.selectExpr("label-1922.0 as label", "features")
...
# TODO: Replace <FILL IN> with appropriate code
average_train_year = (parsed_train_data_df.agg({"label": "avg"})).first()["avg(label)"] #selectExpr("avg(label) as avg")).collect()
print average_train_year
#average_train_year = 54.0403195838

# COMMAND ----------

# TEST Average label (2a)
# 54.0403195838
Test.assertTrue(np.allclose(average_train_year, 54.0570419918),
...
# TODO: Replace <FILL IN> with appropriate code
preds_and_labels_train = parsed_train_data_df.rdd.map(lambda r: (average_train_year, r.label))
...
preds_and_labels_val = parsed_val_data_df.rdd.map(lambda r: (average_train_year, r.label))
...
preds_and_labels_test = parsed_test_data_df.rdd.map(lambda r: (average_train_year, r.label))
...
print 'Baseline Train RMSE: ',rmse_train_base
print 'Baseline Validation RMSE: ',rmse_val_base
print 'Baseline Test RMSE: ',rmse_test_base

# COMMAND ----------

# TEST Training, validation and test RMSE (2c)
# [21.4303303309, 20.9179691056, 21.828603786]
Test.assertTrue(np.allclose([rmse_train_base, rmse_val_base, rmse_test_base],
                            [21.2738400518, 21.9507261745, 22.0420743749]), 'incorrect RMSE values')
...
def gradient_summand(weights, lp):
    ...
    """
    return np.dot(np.dot(lp.features.toArray(), weights) - lp.label, lp.features.toArray())
...
def get_labeled_prediction(weights, observation):
    ...
    """
    return (float(np.dot(weights, observation.features.toArray())), observation.label)
...
def linreg_gradient_descent(train_data, num_iters):
    ...
    for i in range(num_iters):
        ...
        gradient = np.dot(2, np.sum(train_data.map(lambda x: gradient_summand(DenseVector(w), x)).collect(), axis=0))
        
        # Update the weights
        alpha_i = alpha / (n * np.sqrt(i+1))
        w -= np.dot(alpha_i, gradient)
...
expected_output =  [14.32702591, 9.38369699, 17.00686933]
Test.assertTrue(np.allclose(example_weights, expected_output), 'value of example_weights is incorrect')
expected_error = [18.46076921, 3.51646384, 1.912591, 1.84276234, 1.83367961]
...
preds_and_labels = (parsed_val_data_df.rdd.map(lambda x: get_labeled_prediction(weights_LR0, x)))
preds_and_labels_df = sqlContext.createDataFrame(preds_and_labels, ["prediction", "label"])
...
Test.assertTrue(np.allclose(weights_LR0, weights_LR0), 'incorrect value for weights_LR0')
# expected_output
...
coeffs_LR1 = first_model.coefficients[0]
intercept_LR1 = first_model.coefficients
...
# 15.3130800661
Test.assertTrue(np.allclose(rmse_val_LR1, 15.1242987482), 'incorrect value for rmseValLR1')
...
# TEST Grid search (4d)
# 15.3052663831
Test.assertTrue(np.allclose(15.1242987482, rmse_val_LR_grid), 'incorrect value for rmseValLRGrid')
...
def two_way_interactions(lp):
    ...
    """
    features = list(lp.features.toArray())
    inter_features = map(lambda t: t[0]*t[1], list(itertools.product(features, repeat=2)))
    newlp = LabeledPoint(lp.label, features + inter_features)
    return newlp
...
train_data_interact_df = (
  sqlContext.createDataFrame(
    parsed_train_data_df.map(lambda lp: (lp.label, two_way_interactions(lp).features)),
    ["label", "features"]
 )
)
val_data_interact_df = (
  sqlContext.createDataFrame(
    parsed_val_data_df.map(lambda lp: (lp.label, two_way_interactions(lp).features)),
    ["label", "features"]
  )
)
test_data_interact_df = (
  sqlContext.createDataFrame(
    parsed_test_data_df.map(lambda lp: (lp.label, two_way_interactions(lp).features)),
    ["label", "features"]
  )
)
...
# 14.3495530997
Test.assertTrue(np.allclose(rmse_val_interact, 14.697251460074), 'incorrect value for rmse_val_interact')
...
# 14.9990015721
Test.assertTrue(np.allclose(rmse_test_interact, 15.23274242206),
...
# 14.99415450247963 
Test.assertTrue(np.allclose(rmse_test_pipeline, 15.23439010309),
