# Databricks notebook source exported at Thu, 1 Sep 2016 18:06:56 UTC
# MAGIC %md
...
#sample_data_df.map(lambda x: x.color).collect()
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    """
    indices = sorted(map(lambda raw: ohe_dict_broadcast.value[raw], raw_feats))
    values = [1. for i in indices]
    return SparseVector(num_ohe_feats, indices, values)

# Calculate the number of features in sample_ohe_dict_manual
num_sample_ohe_feats = 7
...
sample_distinct_feats_df = (
  sample_data_df.select(
    explode(
      sample_data_df
      .features
      .alias("features")
...
Test.assertEquals(sorted(map(lambda r: (r[0]._1, r[0]._2), sample_distinct_feats_df.collect())), #r[0]
...
def create_one_hot_dict(input_df):
    ...
    """
    return (input_df
     .select(explode(input_df.features)).distinct()
     .rdd.map(lambda r: tuple(r[0])).sortByKey().zipWithIndex().collectAsMap())
...
def parse_point(point):
    ...
    return [(i, idx) for i, idx in enumerate(features)]
...
def parse_raw_df(raw_df):
    ...
    """
    df = raw_df.withColumn("label", split("text", ",").getItem(0).cast("double"))
    df = df.withColumn("feature", parse_point_udf("text").alias("feature")).drop("text").cache()
    
    return df

# Parse the raw training DataFrame
parsed_train_df = parse_raw_df(raw_df)
...
#print num_categories[32][1]


# COMMAND ----------

print num_categories[12]
...
Test.assertEquals(num_categories[2][1], 1888, 'incorrect implementation of parse_point or parse_raw_df')
...
# TODO: Replace <FILL IN> with appropriate code
ctr_ohe_dict = create_one_hot_dict(parsed_train_df.withColumn("features", col("feature")))
...
# 233941
Test.assertEquals(num_ctr_ohe_feats, 273531, 'incorrect number of features in ctr_ohe_dict')
...
ohe_train_df = (parsed_train_df
                  .select("label", ohe_dict_udf("feature")))

ohe_train_df = ohe_train_df.withColumnRenamed("PythonUDF#<lambda>(feature)", "features")
ohe_train_df.cache()
...
Test.assertTrue('label' in ohe_train_df.columns or 'feature' in ohe_train_df.columns, 'ohe_train_df should have label and feature columns')
...
ohe_dict_missing_udf = udf(lambda x: one_hot_encoding(x, ohe_dict_broadcast, len(x)), VectorUDT())
parsed_val_df = parse_raw_df(raw_validation_df)
ohe_validation_df = parsed_val_df.select(ohe_dict_missing_udf("feature").alias("features"))
...
# -1.23057928975
Test.assertTrue(np.allclose(lr_model_basic.intercept,  -1.22801728763), 'incorrect value for model intercept')
Test.assertTrue(np.allclose(sorted_coefficients,
                           [-0.11067035980734835, -0.10264536166888948, -0.10153176968392501, -0.10135172126716652, -0.099188281771865297]),
...
def add_log_loss(df):
    ...
    def bonderies(row):
      newp = row.p
      if(row.p == 0.0 or row.p == 1.0):
        newp = epsilon if row.p == 0.0 else 1.0 - epsilon
        
      return Row(p=newp, label=row.label)
...
class_one_frac_train = np.mean(ohe_train_df.map(lambda y: y.label).collect())
print 'Training class one fraction = {0:.3f}'.format(class_one_frac_train)

log_loss_tr_base = (-1)* np.log(class_one_frac_train)
...
expected_frac = 0.22663
expected_log_loss = 1.48443654727
...
def add_probability(df, model):
    ...
    def get_p(features):
        ...
        raw_prediction = np.dot(features, coefficients_broadcast.value)
        # Bound the raw value between 20 and -20
        raw_prediction = 20 * np.sign(raw_prediction) if np.abs(raw_prediction) > 20 else raw_prediction
...
expected_log_loss = log_loss_train_model_basic = 0.474685722542
...
expected_res = res = 0.6931471805600546
...
expected_val_base = log_loss_val_base = 0.544167185783
...
expected_val_model_basic = log_loss_val_l_r0 = 0.487967958915
